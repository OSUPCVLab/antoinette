...
#
    ...
    def get_data(self,  frame_number):
        ...
        flair_path = self.info['input'][frame_number]
        segment_path = self.info['output'][frame_number]
        flair = nib.load(flair_path)
        flair = np.asarray(flair.get_data()).astype(np.float16)[:,:,11:139]
        # flair = np.pad(flair,(0,16), 'constant')[:,:,11:139]


        segment = nib.load(segment_path)
        segment = np.asarray(segment.get_data()).astype(np.float16)[1:,:,11:139]
        # segment = np.pad(segment,(0,16), 'constant')[:,:,11:139]



        segment =  self.distance_transform(segment, mode ='signed')
        for i in range(64):
            segment[:,:,i] = cv2.normalize(segment[:,:,i],  0, 255, cv2.NORM_MINMAX)
        cv2.imshow('s',segment[:,:,32])
        Visualizer_3D().visualize_3d_volume(segment)

        cv2.waitKey(0)
        return segment, flair
