...
def prepare_data(dataset_dir):
    ...
    test_input_names=[]
    test_output_names=[]
    for file in os.listdir(PureWindowsPath(dataset_dir + "/images/train")):
        cwd = os.getcwd()
        train_input_names.append(PureWindowsPath(cwd + "/" + dataset_dir + "/images/train/" + file))

    for file in os.listdir(PureWindowsPath(dataset_dir + "/annotations/train")):
        cwd = os.getcwd()
        train_output_names.append(PureWindowsPath(dataset_dir + "/annotations/train/" + file))

    for file in os.listdir(PureWindowsPath(dataset_dir + "/images/val")):
        cwd = os.getcwd()
        val_input_names.append(PureWindowsPath(cwd + "/" + dataset_dir + "/images/val/" + file))

    for file in os.listdir(PureWindowsPath(dataset_dir + "/annotations/val")):
        cwd = os.getcwd()
        val_output_names.append( PureWindowsPath(dataset_dir + "/annotations/val/" + file))

    for file in os.listdir(PureWindowsPath(dataset_dir + "/images/test")):
        cwd = os.getcwd()
        test_input_names.append(PureWindowsPath(cwd + "/" + dataset_dir + "/images/test/" + file))

    for file in os.listdir(PureWindowsPath(dataset_dir + "/annotations/test")):
        cwd = os.getcwd()
        test_output_names.append(PureWindowsPath(dataset_dir + "/annotations/test/" + file))
...
class Stacker:
    ...
    def generate_distance_transform(self, image_info, frame_number):
        ...
        print(image_info[frame_number]['path'])
        ...
        for j in range(len(image_info[frame_number]['annotations'])):
            ...
            for i in range(len(torso_links)-1):
        ...
        #1.b) & 2.a) continue
        img_final = cv2.resize(img_final,(64,64))#, fx=0.5, fy=0.5)
        ...
        img = cv2.resize(img,(64,64))#, fx=0.5, fy=0.5)
    ...
    def vectorize_stack_images_2d_temporal(self, seq):
        """ Vectorize and Stack a sequence of images """
        #TODO: change this from config file
        img_output = np.zeros((32, 4096), dtype = np.uint8)#self.time_length
        img_input = np.zeros((32, 4096,3), dtype = np.uint8)
        ...
        for i in range(self.time_length):
            ...
            # img_i = image_serialization([64,64,3],img_i)
            # img_i = np.reshape(img_i, (1,img_i.shape[0]*img_i.shape[1],3))
            # img_o = np.reshape(img_o, (1,img_o.shape[0]*img_o.shape[1]))
            # img_output[i] = img_o
            # img_input[i] = img_i
        ...
        #     img_o =  image_serialization([64,64,1],img_o)
        #     img_o = np.reshape(img_o, (1,img_o.shape[0]*img_o.shape[1]))
        #     img_output[i] = img_o
        ...
        # cv2.imshow('input',img_input[19,:,:].reshape(64,64,3))
        # cv2.imshow('mask', cv2.normalize(img_input_3d[:,20,:],  0, 255, cv2.NORM_MINMAX))
        # cv2.waitKey(0)

        return img_output, img_input
