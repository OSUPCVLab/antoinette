...
from utils import utils3 as utils, helpers
...
def main():
    ...
    time_length = 128
    train_lab, val_lab, test_lab = utils.prepare_data(os.path.join(base_dir , "Data\\BraST2018"), time_length,mode = 'TRAIN')
    ...
    num_classes =  2
    net_input =  tf.placeholder(tf.float32,shape=[None,None,None,None,1])
    net_output = tf.placeholder(tf.float32,shape=[None,None,None,None,1])#,None,num_classes
    ...
    for epoch in range(EPOCHS):
        ...
        for i in num_iters:
            ...
            for j in range(batch_size):
                ...
                with tf.device('/cpu:0'):
                    ...
                    img_input = np.float32(img_input) /np.max(img_input)
                    img_input = np.float32(np.expand_dims(img_input, axis = 3))
        ...
        if epoch % 1 == 0:
            ...
            for ind in val_indices:
                ...
                output_image = sess.run(network, feed_dict = {net_input: input_image/np.max(input_image)})
