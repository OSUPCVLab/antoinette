...
def main():
    ...
    for ind in range(0,len(test_lab['input'])):
            ...
            for i in range(time_length):
                ...
                file_name = os.path.basename(test_lab['input'][ind][0])
                file_name = os.path.splitext(file_name)[0]

                cv2.imwrite(os.path.join(base_dir ,"%s\\%04d\\%s1_%04d_pred.png"%("results",ind, file_name,i)),np.uint8(output_image_i))
                cv2.imwrite(os.path.join(base_dir ,"%s\\%04d\\%s2_%04d_gt.png"%("results",ind, file_name,i)),np.uint8(gt_i))
                cv2.imwrite(os.path.join(base_dir ,"%s\\%04d\\%s3_%04d_gt_img.png"%("results",ind, file_name,i)),np.uint8(input_image_i))
                p = output_image_i < 3
                ...
                cv2.imwrite(os.path.join(base_dir ,"%s\\%04d\\%s5_%04d_sub2.png"%("results",ind, file_name,i)),np.uint8(input_image_i))
