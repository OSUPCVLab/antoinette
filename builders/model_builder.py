...
SUPPORTED_MODELS = ["FC-DenseNet56", "FC-DenseNet67", "FC-DenseNet103", "Encoder-Decoder", "Encoder-Decoder-Skip", "RefineNet",
    ...
    "DenseASPP", "DDSC", "BiSeNet", "Siamese-Encoder-Decoder","Siamese-Encoder-Decoder-Skip",
    "Encoder-Decoder-3D","Encoder-Decoder-Skip-3D","custom"]

SUPPORTED_FRONTENDS = ["ResNet50", "ResNet101", "ResNet152", "MobileNetV2", "InceptionV4"]

def download_checkpoints(model_name):
    print(os.path.join(os.getcwd(), "\\utils\\get_pretrained_checkpoints.py"))
    subprocess.Popen([sys.executable, os.path.join(os.getcwd(), "\\utils\\get_pretrained_checkpoints.py"), "--model=" + model_name]).communicate()
...
def build_model(model_name, net_input, num_classes,frontend="ResNet101", is_training=True):
    ...
    elif model_name == "Siamese-Encoder-Decoder" or model_name == "Siamese-Encoder-Decoder-Skip":
